...
/**
 * https://github.com/catfan/Medoo/blob/15a53403baa84b3c288f0f9419aa6338acbb82da/src/Medoo.php
 *
 * 单独引用类 防止和其他框架冲突
 */
/**
...
namespace Baiy\Cadmin\Library\Medoo;
...
class Medoo
{
    ...
    protected function columnPush(&$columns, array &$map, bool $root, bool $isJoin = false): string
    {
        ...
        foreach ($columns as $key => $value) {
            ...
            if (!$isIntKey && $isArrayValue && $root && count(array_keys($columns)) === 1) {
                ...
                preg_match('/(?<column>[\p{L}_][\p{L}\p{N}@$#\-_\.]*)(?:\s*\((?<alias>[\p{L}_][\p{L}\p{N}@$#\-_]*)\))?(?:\s*\[(?<type>(?:String|Bool|Int|Number|Object|JSON))\])?/u',
                    $value, $match);
    ...
    protected function columnMap($columns, array &$stack, bool $root): array
    {
        ...
        foreach ($columns as $key => $value) {
            if (is_int($key)) {
                preg_match('/([\p{L}_][\p{L}\p{N}@$#\-_]*\.)?(?<column>[\p{L}_][\p{L}\p{N}@$#\-_]*)(?:\s*\((?<alias>[\p{L}_][\p{L}\p{N}@$#\-_]*)\))?(?:\s*\[(?<type>(?:String|Bool|Int|Number|Object|JSON))\])?/u',
                    $value, $keyMatch);
                ...
                preg_match('/([\p{L}_][\p{L}\p{N}@$#\-_]*\.)?(?<column>[\p{L}_][\p{L}\p{N}@$#\-_]*)(\s*\[(?<type>(String|Bool|Int|Number))\])?/u', $key,
                    $keyMatch);
